...
static void* network_watch_task(void* arg)
{
    ...
    /* HomeMind fix: do NOT abort after 30s.  The old code gave up on the
     * first timeout and never started agent_loop, so any session that
     * started the agent before Wi-Fi came up (no saved credentials yet, or
     * user runs set_wifi later) left 'ask' silently queued forever.  Poll
     * every 5s until the link is up or shutdown is requested. */
    bool net_timeout_logged = false;
    while (network_wait_connected(5000) != OK) {
        if (agent_shutdown_requested()) {
            return NULL;
        }
        if (!net_timeout_logged) {
            syslog(LOG_WARNING,
                   "[%s] Network not up yet — waiting for Wi-Fi "
                   "(set_wifi or saved credentials)\n",
                   TAG);
            net_timeout_logged = true;
        }
    }
...
int ai_agent_main(int argc, char* argv[])
{
    ...
    /* LCD status icons (Wi-Fi / MiMo / LED) — polls states itself */
    if (hm_lcd_display_start() != OK) {
        syslog(LOG_WARNING, "[%s] hm_lcd_display_start failed\n", TAG);
    }
    BOOT_LOG(&t0, "P5", "lcd status display started");
