...
/* HomeMind: monotonic wall-time wrapper for latency measurement.
 * gettimeofday() jumps when SNTP adjusts the clock (board has no RTC),
 * which made the LLM watchdog measure multi-year latencies and discard
 * valid responses.  All agent_loop latency probes use this instead. */
...
static const char* kw_led_on[]     = { "打开灯", "开灯", "turn on the light",
                                      "turn on the led", "light on", NULL };
static const char* kw_led_off[]    = { "关灯", "关闭灯", "关掉灯",
                                      "turn off the light",
                                      "turn off the led", "light off", NULL };
...
static const nl_intent_t s_intents[] = {
    { kw_led_on,    "led_control",      "{\"action\":\"on\"}",  128 },
    { kw_led_off,   "led_control",      "{\"action\":\"off\"}", 128 },
    ...
    const agent_msg_t* msg)
{
    ...
    for (iteration = 0; iteration < AGENT_AI_AGENT_MAX_TOOL_ITER;
        ...
        if (err != OK) {
            ...
            syslog(LOG_ERR, "[%s] LLM call failed (iter %d) err=%d latency=%u ms\n",
                TAG, iteration, err, (unsigned)latency_ms);
