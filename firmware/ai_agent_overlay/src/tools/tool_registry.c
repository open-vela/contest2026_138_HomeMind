...
int tool_registry_init(void)
{
    ...
    REGISTER_TOOL(
        "led_control",
        "Control the onboard LED. Use this whenever the user asks to turn "
        "the light/LED on or off, or to check its state.",
        TOOL_SCHEMA_BEGIN()
            TOOL_PARAM_ENUM("action", "LED action",
    ...
    REGISTER_TOOL_NO_PARAMS(
        "get_device_info",
        "Get device status: uptime seconds, free/used heap, current IP, "
        "firmware build. Use for questions like how long has the device "
        "been running, how much memory is free, or what is its IP.",
        tool_get_device_info_execute);

    REGISTER_TOOL_NO_PARAMS(
        "get_button",
        "Read the BOOT button state (pressed or released).",
        tool_get_button_execute);
