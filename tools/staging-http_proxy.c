...
int http_proxy_init(void)
{
    ...
    if (s_proxy_host[0] && s_proxy_port) {
        syslog(LOG_INFO, "[HM-NET] Proxy configured: %s:%d (proxy ENABLED)\n", s_proxy_host, s_proxy_port);
    } else {
        syslog(LOG_INFO, "[HM-NET] No proxy configured — using DIRECT connection\n");
    }
    return OK;
}
...
static int open_connect_tunnel(const char *host, int port, int timeout_ms)
{
    ...
    syslog(LOG_INFO, "[HM-NET] Proxy tunnel: TCP connect to proxy %s:%d\n", s_proxy_host, s_proxy_port);
    ...
    if (rc != 0 && errno != EINPROGRESS) {
        syslog(LOG_ERR, "[HM-NET] Proxy TCP connect FAILED: %s:%d, errno=%d (%s)\n",
               s_proxy_host, s_proxy_port, errno, strerror(errno));
        close(sock);
        return -1;
    }
    ...
    if (rc <= 0) {
        syslog(LOG_ERR, "[HM-NET] Proxy TCP connect TIMEOUT: %s:%d\n",
               s_proxy_host, s_proxy_port);
        close(sock);
        return -1;
    }
    ...
    if (err != 0) {
        syslog(LOG_ERR, "[HM-NET] Proxy TCP connect FAILED: %s:%d, SO_ERROR=%d\n",
               s_proxy_host, s_proxy_port, err);
        close(sock);
        return -1;
    }

    /* Always restore blocking mode for the TLS BIO.  Retaining O_NONBLOCK
     * turns a transient EAGAIN into MBEDTLS_ERR_NET_RECV_FAILED during the
     * handshake on ESP32-S3/NuttX. */
    fcntl(sock, F_SETFL, flags & ~O_NONBLOCK);
