...
Ubuntu 主机、OpenVela 工作区、固定依赖、串口权限和服务器地址 `192.168.31.251` 均已验证可用。2026-09-01 修复版干净构建已烧录到 ESP32-S3-EYE：设备锁定强 AP `50:88:11:7a:02:69`，通过 DHCP 获得 `192.168.31.248`；冷启动 5 轮均完成 DHCP/TLS/HTTP，标准自然语言问答已完成 7/7 轮通过，说明此前间歇超时问题已有修复证据；仍需补足 10 轮、复核异常路径并本地提交。远程推送尚未执行。
...
1. 修复版已完成干净构建、烧录、冷启动 5 轮及标准自然语言问答 7/7 轮；长稳 10 轮和受控重连复核待明日完成；
2. 设备 Flash 中已保存用户提供的 MiMo key，提交物和日志不含完整 key；后续应按需要轮换设备侧凭据；
3. 当前本地官方仓包含 TLS 源码、staging、snapshot、暂停记录和脱敏硬件日志改动，尚未提交、尚未推送。
...
| 自动测试和真实日志 | 修复前基线 6/10；2026-09-01 修复版自然语言连续回归已完成 7/7 PASS，冷启动传输链 5/5 无超时；记录已保存 | 补足修复版 10 轮，复核异常路径并提交 |
...
1. 补足修复版标准自然语言长稳 10 轮，并复核受控 `wifi_reconnect`；
2. 复核 TLS body framing、异常响应和重复请求路径的串口日志；
3. 本地提交源码、快照、staging、暂停记录和脱敏硬件日志；
