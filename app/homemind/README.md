...
`skills/home_security.md` 是比赛仓库中的运行时 Skill 源文件；设备实际加载路径由固件的 `AGENT_SKILLS_DIR` 决定，本机已验收固件使用 `/data/ai_agent/skills/`。`configs/default_config.json` 仍保留早期 FastAPI/MQTT 字段，仅作历史设计参考，不代表当前独立版运行配置。
